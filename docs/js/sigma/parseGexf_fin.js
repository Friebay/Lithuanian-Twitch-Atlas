...
sigma.publicPrototype.parseGexf = function (gexfPath, callback) {
  ...
  gexfhttp.onreadystatechange = function () {
    if (gexfhttp.readyState == 4 && (gexfhttp.status == 200 || gexfhttp.status == 0)) {
      ...
      for (i = 0; i < edgesNodes.length; i++) {
        ...
        for (j = 0; j < edgeNodes.length; j++) {
          ...
          for (var i_attr = 0; i_attr < attrs.length; i_attr++) {
            var n = attrs[i_attr].name;
            if (n == 'source' || n == 'target' || n == 'label')
              continue;
            edge.attributes[n] = attrs[i_attr].value;
          }
  ...
  gexfhttp.open('GET', gexfPath, true);
  gexfhttp.send();
};
